...
public class CartController {
    ...
    public ResponseEntity<ApiResponse<CartResponse>> addItem(@Valid @RequestBody CartItemRequest request) {
        return ResponseEntity.ok(ApiResponse.success("Item added to cart", cartService.addItem(SecurityUtils.getCurrentUserId(), request)));
    }

    @PutMapping("/item/{cartItemId}")
    public ResponseEntity<ApiResponse<CartResponse>> updateItem(@PathVariable Long cartItemId, @RequestParam int quantity) {
        return ResponseEntity.ok(ApiResponse.success("Cart item updated", cartService.updateItemQuantity(SecurityUtils.getCurrentUserId(), cartItemId, quantity)));
    }

    @DeleteMapping("/item/{cartItemId}")
    public ResponseEntity<ApiResponse<CartResponse>> removeItem(@PathVariable Long cartItemId) {
        return ResponseEntity.ok(ApiResponse.success("Cart item removed", cartService.removeItem(SecurityUtils.getCurrentUserId(), cartItemId)));
    }
